...
bool Context::Init()
{
    ...
    m_vertexBuffer = Buffer::CreateWithData(GL_ARRAY_BUFFER, GL_STATIC_DRAW, vertices, sizeof(float) * 120); //8 col * 4 raw =32 float size
...
{
    if (!m_cameraControl) //when the m_cameraControl valuse is false nothing happen
        return;
...
void Context::Render()
{
    ...
    auto view = glm::lookAt(
                            m_cameraPos,
                            m_cameraPos + m_cameraFront, //camera tartget + camera Front
                            m_cameraUp);    
